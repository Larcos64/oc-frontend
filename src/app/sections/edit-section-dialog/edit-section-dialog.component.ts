...
export class EditSectionDialogComponent implements OnInit {
  ...
  constructor(private router: Router, private fb: FormBuilder, public dialogRef: MatDialogRef<EditSectionDialogComponent>,
    private snackbar: MatSnackBar, private route: ActivatedRoute, @Inject(MAT_DIALOG_DATA) public data: any, public service: SectionsService) { }
  ...
  initForm() {
    this.form = this.fb.group({
      ...
      nameSection: [this.data.nameSection, [Validators.required, Validators.maxLength(200)]],
      descSection: [this.data.descSection, Validators.maxLength(200)],
      typeSection: [this.data.typeSection, Validators.maxLength(10)],
      tableName:   [this.data.tableName, [Validators.required, Validators.maxLength(20)]],
  ...
  editSection() {
    ...
    this.loading = true;
    (this.service.edit(this.section)).pipe(
      finalize(() => this.loading = false)
    ).subscribe(() => this.editOk(), () => snackError(this.snackbar, 'Error al editar sección'));
  }
