...
export class ListVersionsComponent implements OnInit {
  ...
  permAssignSections = this.servicePermits.validatePermit(
    "Encuestas.versiones.asignarSecciones"
  );
  permAssignQuestions = this.servicePermits.validatePermit(
    "Encuestas.versiones.asignarPreguntas"
  );
  permCreate = this.servicePermits.validatePermit("Encuestas.versiones.crear");
  permEdit = this.servicePermits.validatePermit("Encuestas.versiones.editar");
  permDel = this.servicePermits.validatePermit("Encuestas.versiones.eliminar");
  ...
  constructor(
    ...
    public serviceForm: FormatsService
  ) {}

  ngOnInit() {
    // pedir explicación de luis
  ...
  startDelete(i: number, element: any) {
    ...
    this.confirmDialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      closeOnNavigation: true,
      data: {
        message: `¿Esta seguro de eliminar la versión con código: ${element.cod_version}?`,
      },
    });

    this.confirmDialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.service.delete(element.id_version).subscribe(
          () => {
  ...
  startEditSec(i: number, element: any) {
    this.index = i;
    this.versionSectionDialog = this.dialog.open(
      VersionSectionsDialogComponent,
      {
        width: "80%",
        height: "auto",
        closeOnNavigation: true,
