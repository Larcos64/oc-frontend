...
export class AddVersionDialogComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any, private router: Router, private fb: FormBuilder, public service: VersionsService,
    private snackbar: MatSnackBar, private route: ActivatedRoute, public dialogRef: MatDialogRef<AddVersionDialogComponent>,
  ...
  saveVersion() {
    ...
    this.loading = true;
    if (this.edit) {
      this.service.edit(this.obj).pipe(
        finalize(() => this.loading = false)
      ).subscribe(() => this.addOk(), (err) => snackError(this.snackbar, err));
    } else
      this.service.add(this.obj).pipe(
        finalize(() => this.loading = false)
      ).subscribe(() => this.addOk(), (err) => snackError(this.snackbar, err));
  }
  ...
  addOk() {
    if (this.edit)
      snackOk(this.snackbar, 'Versión editada')
    else
      snackOk(this.snackbar, 'Versión registrada')
    this.router.navigate([`home/versions/id_format/${this.data.idFormat}`], { relativeTo: this.route });
