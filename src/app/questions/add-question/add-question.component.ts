...
export class AddQuestionComponent implements OnInit {
  ...
  listTypes = [{ value: 'short_text', type: 'Respuesta Corta', icon: 'short_text' },
  ...
  constructor(private router: Router, private service: QuestionsService, private activateRoute: ActivatedRoute, private fb: FormBuilder,
    public dialog: MatDialog, private snackbar: MatSnackBar, private route: ActivatedRoute, private serviceOpt: OptionsService,
  ...
  get qfg() { return this.questionFormGroup.controls; }

  // convenience getter for easy access to form fields
  get ofg() { return this.optionFormGroup.controls; }
  ...
  saveOption() {
    ...
    this.loading = true;
    (this.serviceOpt.add(this.option)).pipe(
  ...
  saveOptValues(dataOption, object) {
    ...
    (this.serviceOptVal.add(this.optval)).pipe(
      finalize(() => this.loading = false)
    ).subscribe(() => this.saveQuestion(dataOption), (err) => snackError(this.snackbar, err));
  }
