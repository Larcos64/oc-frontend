...
export class EditDependencyDialogComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: any, private fb: FormBuilder, public service: DependenciesService,
    ...
    public serviceOptVal: OptvaluesService, private snackbar: MatSnackBar, public dialogRef: MatDialogRef<EditDependencyDialogComponent>) { }

  ngOnInit() {
    this.getParentQuestion(this.data.id_ques);
    this.initForm();
    this.getSections();
  }
  ...
  initForm() {
    this.form = this.fb.group({
      idDep: [this.data.id_dep],
      secQues: [],
      idQues: [this.data.id_ques, Validators.required],
      idSec: [],
      idQues2: [],
      operatorDep: [this.data.operator_dep],
      optValues: [parseInt(this.data.value_dep)],
      valueDep: [parseInt(this.data.value_dep)],
      orDep: [this.orDep]
    });

    if (this.data.id_ques2) {
      this.form.controls.idQues2.setValue(this.data.id_ques2);
      this.disFields = ['idSec'];
      this.disableFields(this.disFields);
    } else if (this.data.id_sec_dep) {
      this.form.controls.idSec.setValue(this.data.id_sec_dep);
  ...
  getParentQuestion(idQues) {
    this.serviceQues.quesById(idQues).subscribe(question => {
      ...
      this.form.controls.secQues.setValue(this.parentquestion['id_sec']);
      this.getQuestions(this.parentquestion['id_sec']);
      this.avQues = true;
      this.idOpt = this.parentquestion['id_opt'];
      this.typeQues = this.parentquestion['type_ques'];
      if (this.typeQues === 'money') {
  ...
  buildFields(idOpt, typeQues) {
    if (typeQues === 'money') {
      ...
      // this.form.controls.optValues.setValue(this.data.value_dep);
  ...
  editDependency() {
    ...
    this.loading = true;
    (this.service.edit(this.dependency)).pipe(
      finalize(() => this.loading = false)
    ).subscribe(() => this.editOk(), (err) => snackError(this.snackbar, err));
  }

  editOk() {
    snackOk(this.snackbar, 'Dependencia actualizada');
