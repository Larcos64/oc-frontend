...
export class ListFormatsComponent implements OnInit {
  ...
  permAssignSections = this.servicePermits.validatePermit('Formatos.asignarSecciones');
  permAssignToCompanies = this.servicePermits.validatePermit('Formatos.asignarEmpresas');
  ...
  constructor(private router: Router, private service: FormatsService, public dialog: MatDialog, public servicePermits: PermitsService) { }
  ...
  startEdit(i: number, idFormat: number, idUser: number, nameFormat: string, descFormat: string, typeFormat: string, codFormat: string,
    issueDate) {
    this.index = i;
    this.editDialogRef = this.dialog.open(EditFormatDialogComponent, {
      ...
      autoFocus: false,
      data: {
        idFormat, idUser, nameFormat, descFormat, typeFormat, codFormat, issueDate
      }
    });
  ...
  startDelete(i: number, idFormat: number, nameFormat: string, descFormat: string, codFormat: string) {
    this.index = i;
